...
let make_time_filters ~scope_name update_time_filter period =
  (match period, update_time_filter with
   | (None, None), None -> []
   | (_, _), None ->
...
let process_scope ~config ~period ~ldap_conn ~target_cache scope_name =
  ...
  let rec recurse period partition_lb partition_ub =
    let filter =
      (match make_time_filters ~scope_name update_time_filter period with
       | [] -> fixed_filter
    ...
    Log.debug (fun f ->
      f "LDAP base: %s" scope.Config.ldap_base_dn) >>= fun () ->
    Log.debug (fun f ->
      f "LDAP scope: %s" (Netldapx.string_of_scope scope.Config.ldap_scope))
      >>= fun () ->
    Log.debug (fun f ->
     ...
     | `SizeLimitExceeded ->
        (match period, scope.Config.ldap_partition_attribute_type with
         | ((Some (tI, tzI) as tI'), (Some (tF, _) as tF')), _ when
            ...
            (match%lwt recurse (tI', tM') None None with
             | None -> recurse (tM', tF') None None
             ...
             | `Entry (_, attrs) ->
                (match List.assoc_opt attr_type attrs with
                 | Some (partition :: _) ->
                    ...
                    (match%lwt recurse period partition_lb (Some partition) with
                     | None -> recurse period (Some partition) partition_ub
                     | Some err -> Lwt.return_some err)
                 | Some [] | None ->
                    Log.err (fun f ->
